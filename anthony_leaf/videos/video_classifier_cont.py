# leaf_count_regressor_padded_int_wandb.py
...
# Initialize wandb
wandb.init(project="leaf_count_classifier", name="padded_int_regression_with_acc", config={
    "batch_size": 3,
    "lr": 1e-3,
    "epochs": 30,
    "model": "LeafRegressor_padded_int_with_acc"
})
config = wandb.config
...
class LeafVideoDataset(Dataset):
    def __init__(self, labels_csv, npy_dir):
        ...
        with open(labels_csv, 'r') as f:
            ...
            for row in reader:
                name = row['name']
                leaves = float(row['leaves'])
                diseased = float(row['diseased'])
...
class LeafRegressor(nn.Module):
    def __init__(self):
        super(LeafRegressor, self).__init__()
    ...
    def forward(self, x):
        ...
        out_leaves = self.fc_leaves(x).squeeze(1)  # shape (B,)
        out_diseased = self.fc_diseased(x).squeeze(1)  # shape (B,)
...
for epoch in range(config.epochs):
    ...
    # Training metrics
    model.eval()
    train_mae_leaves = 0.0
    train_mae_diseased = 0.0
    ...
    with torch.no_grad():
        for data, targets in train_loader:
            ...
            train_mae_leaves += torch.abs(outputs_leaves - targets[:,0]).sum().item()
            train_mae_diseased += torch.abs(outputs_diseased - targets[:,1]).sum().item()

            preds_leaves = torch.round(outputs_leaves)
            preds_diseased = torch.round(outputs_diseased)
    ...
    train_mae_leaves /= len(train_loader.dataset)
    train_mae_diseased /= len(train_loader.dataset)
    ...
    with torch.no_grad():
        for data, targets in tqdm(val_loader, desc=f"Epoch {epoch+1}/{config.epochs} - Validation"):
            ...
            val_mae_leaves += torch.abs(outputs_leaves - targets[:,0]).sum().item()
            val_mae_diseased += torch.abs(outputs_diseased - targets[:,1]).sum().item()
    ...
    val_mae_leaves /= len(val_loader.dataset)
    val_mae_diseased /= len(val_loader.dataset)
    ...
    wandb.log({
        ...
        "train_mae_leaves": train_mae_leaves,
        "train_mae_diseased": train_mae_diseased,
        "train_leaves_acc": train_leaves_acc,
        "train_diseased_acc": train_diseased_acc,
        "train_exact_acc": train_exact_acc,
        "val_loss": val_loss,
        "val_mae_leaves": val_mae_leaves,
        "val_mae_diseased": val_mae_diseased,
        "val_leaves_acc": val_leaves_acc,
        "val_diseased_acc": val_diseased_acc,
        "val_exact_acc": val_exact_acc
    })

    print(f"Epoch {epoch+1}: Train Loss: {train_loss:.4f} | Train MAE Leaves: {train_mae_leaves:.2f} | "
          f"Train MAE Diseased: {train_mae_diseased:.2f} | Train Leaves Acc: {train_leaves_acc:.2f}% | "
          f"Train Diseased Acc: {train_diseased_acc:.2f}% | Train Exact Match Acc: {train_exact_acc:.2f}% || "
          f"Val Loss: {val_loss:.4f} | Val MAE Leaves: {val_mae_leaves:.2f} | Val MAE Diseased: {val_mae_diseased:.2f} | "
          f"Val Leaves Acc: {val_leaves_acc:.2f}% | Val Diseased Acc: {val_diseased_acc:.2f}% | Val Exact Match Acc: {val_exact_acc:.2f}%")
