...
parser = argparse.ArgumentParser(description="Leaf sliding window 3D CNN per video")
...
if use_wandb:
    wandb.init(project="leaf-warmup", name=run_name, config={
        ...
        "optimizer": "Adam",
        "loss": "BCE",
        "architecture": "SlidingWindowVideoCNN3D",
...
# ===== Sliding window 3D CNN Model (per video) =====
class SlidingWindowVideoCNN3D(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        """
        x: (B=1, C, D, H, W) full video
        returns: (B=1, D, 1) per-frame predictions
        """
        B, C, D, H, W = x.shape
        half_window = 3  # window size 7

        padded = torch.zeros(B, C, D + 2*half_window, H, W, device=x.device)
        padded[:, :, half_window:half_window+D] = x

        outputs = []

        for i in range(D):
            window = padded[:, :, i:i+2*half_window+1]  # (B,C,7,H,W)

            # Feature extraction
            feat = self.features(window)  # (B,64,7,32,16)
            feat = feat.view(B, -1)  # (B,64*7*32*16)

            out = self.mlp(feat)  # (B,1)
            outputs.append(out)

        outputs = torch.stack(outputs, dim=1)  # (B, D, 1)
        return outputs

model = SlidingWindowVideoCNN3D().to(device)
