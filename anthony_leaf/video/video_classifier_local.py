...
class LeafFrameDataset3D(Dataset):
    def __init__(self, labels_dict, frames_dir, transform=None, num_neighbors=3):
        self.entries = sorted(labels_dict.items())  # sorted to keep frame order
        self.frames_dir = frames_dir
        self.transform = transform
        self.num_neighbors = num_neighbors

        # Prepare list of all frame filenames for indexing
    ...
    def __getitem__(self, idx):
        ...
        for i in indices:
            if 0 <= i < len(self.entries):
                ...
                image = torch.zeros(3, 640, 360)  # pad with zero frame
            frames.append(image)
        # Stack into shape (T, C, H, W)
...
train_loader = DataLoader(dataset, batch_size=4, shuffle=True)  # smaller batch size due to 3D data
...
class Leaf3DCNN(nn.Module):
    def __init__(self, num_classes=3):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(2800, 1024),  # assuming D=7
            nn.ReLU(),
    ...
    def forward(self, x):
        # Input x shape: (B, T, C, H, W) -> (B, C_new, D, H, W)
