...
def create_trainingdata_bc_dict(exp, exp_sp, exp_robot, device, validation=False):
    """Creates training data for training and validation with the demonstrations specified in the exp/run. Caches the results into the input and target files specified in the exp/run. Remove those files to recalculate."""
    # CHANGES
    # Now it will not shuffle if configured not to, and it will create and use a validation split is configured to do so in exp
        ...
        for key, value in split_paths.items():
            ...
            for val in exp["training_data"]: # for all demonstrations
                ...
                for i in range(demo.metadata["maxsteps"]-1): # -1 because of lookahead
                    ...
                    inputs_list.append(torch.from_numpy(z))
                    # the action we are choosing, is the next one
                    # Return actions as dict instead of list
                    a = demo.get_action_dict(i)
                    # Assign rp actions based on dict key,value pairs instead of from a list
                    # I forget if there was a bug here. I believe there was. Now it's explicit to avoid any bugs.
                    rp = RobotPosition.from_vector_dict(exp_robot, a)
                    # Previously there was an error when converting from and to normalized values during runtime
                    anorm_dict = rp.to_normalized_vector_dict(exp_robot)
                    # Example output: {'height': -0.019999999999999928, 'distance': 0.019999999999999928, 'heading': 0.5, 'wrist_angle': -0.03333333333333333, 'wrist_rotation': 0.006666666666666667, 'gripper': -0.05}        
                    # Converting action dict to a tensor of float32 for training
                    # Note the order: {'height', 'distance', heading', 'wrist_angle', 'wrist_rotation', 'gripper'}        
                    anorm = torch.tensor(list(anorm_dict.values()), dtype=torch.float32)
                    targets_list.append(anorm)
                inputs_tensor = torch.stack(inputs_list)
                targets_tensor = torch.stack(targets_list)
                # at this point we have two tensors that are aligned demonstrations
                all_demos_inputs_tensor, all_demos_targets_tensor = create_trainingpair_prediction(inputs_tensor, targets_tensor, sequence_length=exp["sequence_length"])
